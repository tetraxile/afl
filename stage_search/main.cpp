...
result_t SearchEngine::searchScenario(const byml::Reader& scenario) {
    ...
    for (u32 listIdx = 0; listIdx < scenario.getSize(); listIdx++) {
        ...
        for (u32 itemIdx = 0; itemIdx < itemList.getSize(); itemIdx++) {
            ...
            std::string itemName;
            r = item.getStringByKey(&itemName, "UnitConfigName");
            if (r) return r;

            if (util::isEqual(itemName, mQuery.name)) {
                std::string objId;
...
s32 main(s32 argc, char** argv) {
	if (argc < 3) {
		fprintf(stderr, "usage: %s <game> <romfs path> [output file]\n", argv[0]);
		return 1;
	}

    std::string gameName = argv[1];
    std::string romfsPath = argv[2];
    // std::string outPath = argc < 3 ? "results.txt" : argv[3];
