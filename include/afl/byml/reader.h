...
public:
	...
	bool tryGetContainerByIdx(Reader* container, u32 idx) const { return getContainerByIdx(container, idx) ? false : true; }
	bool tryGetStringByIdx(std::string* out, u32 idx) const { return getStringByIdx(out, idx) ? false : true; }
	...
	bool tryGetContainerByKey(Reader* container, const std::string& key) const { return getContainerByKey(container, key) ? false : true; }
	bool tryGetStringByKey(std::string* out, const std::string& key) const { return getStringByKey(out, key) ? false : true; }
	bool tryGetBoolByKey(bool* out, const std::string& key) const { return getBoolByKey(out, key) ? false : true; }
	bool tryGetS32ByKey(s32* out, const std::string& key) const { return getS32ByKey(out, key) ? false : true; }
	bool tryGetF32ByKey(f32* out, const std::string& key) const { return getF32ByKey(out, key) ? false : true; }
	bool tryGetU32ByKey(u32* out, const std::string& key) const { return getU32ByKey(out, key) ? false : true; }
	bool tryGetS64ByKey(s64* out, const std::string& key) const { return getS64ByKey(out, key) ? false : true; }
	bool tryGetF64ByKey(f64* out, const std::string& key) const { return getF64ByKey(out, key) ? false : true; }
	bool tryGetU64ByKey(u64* out, const std::string& key) const { return getU64ByKey(out, key) ? false : true; }
...
private:
	...
	result_t getNodeByKey(const u8** offset, const std::string& key, NodeType expectedType)
		const;
