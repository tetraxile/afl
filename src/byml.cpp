...
result_t BYML::get_node_by_key(const u8** offset, const std::string& key) const {
...
result_t BYML::get_container_by_key(BYML* container, const std::string& key) const {
	const u8* offset;
	result_t r = get_node_by_key(&offset, key);
	if (r) return r;

	NodeType type = (NodeType)reader::read_u8(offset + 3, mHeader.mByteOrder);
